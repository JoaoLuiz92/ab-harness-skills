...
**SKILL_ROOT:** directory containing this `SKILL.md` (after install, e.g. `~/.cursor/skills/harness-bootstrap` or `.agents/skills/harness-bootstrap`). Run `npx skills list` if unsure.
...
**Validation:** `workflow.config.md` exists; selected tool adapters present; `npm run validation-lane:handoff -- --help` works if Node project.
...
Result: target repo has `workflow.config.md`, `docs/workflow/`, `.cursor/rules/`, and a passing pilot report under `docs/workflow/reports/`.
